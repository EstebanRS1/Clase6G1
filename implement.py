...
def main():
    ...
    while True:
        ...
        if menu == 1:
            if (servicio_hospitalario.verNumerofelino() and servicio_hospitalario.verNumerocanino()) >= 7:
                print("No hay espacio dispnible...")
                continue
            historia = int(input(" ingrese la historia clinica de la mascota: "))
            if (servicio_hospitalario.verificarExisteF(historia) and servicio_hospitalario.verificarExisteC(historia))== False:
                nombre=input("Ingrese el nombre de la mascota: ")
                tipo=input("Ingrese el tipo de mascota (felino o canino): ")
                peso=int(input("Ingrese el peso de la mascota: "))
                fecha=input("Ingrese la fecha de ingreso (dia/mes/año): ")
                nm=int(input("Ingrese la cantidad de medicamentos que se le suministran: "))
                lista_med=[]
                for i in range(nm):
